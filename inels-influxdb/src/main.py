...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            elif unprocessed_queue_size >= 10 and unprocessed_queue_size < 20 or processed_queue_size >= 10 and processed_queue_size < 20:
                logger.warning(utils.queue_log(unprocessed_queue_size, processed_queue_size))
            elif unprocessed_queue_size >= 20 and unprocessed_queue_size < 30 or processed_queue_size >= 20 and processed_queue_size < 30:
                logger.error(utils.queue_log(unprocessed_queue_size, processed_queue_size))
            elif unprocessed_queue_size >= 30 and unprocessed_queue_size < 50 or processed_queue_size >= 30 and processed_queue_size < 50:
                logger.critical(utils.queue_log(unprocessed_queue_size, processed_queue_size))
            elif unprocessed_queue_size >= 50 or processed_queue_size >= 50:
                logger.critical(utils.queue_log(unprocessed_queue_size, processed_queue_size))
                raise KeyboardInterrupt
